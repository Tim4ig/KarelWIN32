...
void print_beeper(const int nr) {
...
void _draw_world() {
  ...
  for (int row = _world.height - 1; row >= 0; row--) {
    ...
    for (int column = 0; column < _world.width; column++) {
      const int block = _world.data[row][column];
      const int left = column - 1 >= 0 ? _world.data[row][column - 1] : WALL;
      const int right = column + 1 < _world.width ? _world.data[row][column + 1] : WALL;
      const int up = row + 1 < _world.height ? _world.data[row + 1][column] : 0;
      const int down = row - 1 >= 0 ? _world.data[row - 1][column] : 0;
      ...
      if (block == WALL) {
        ...
        if (left != WALL && right != WALL && ((up != WALL && down == WALL) || (down != WALL && up == WALL))) {
          printw("| ");
          continue;
        }
 ...
 */
void _update(const int dx, const int dy) {
  if (dx != 0 || dy != 0) {
    const int block = _world.data[_karel.y - 2 * dy][_karel.x - 2 * dx];
...
void _render() {
  ...
  printw(" (%d, %d)   %-5s     %2d        %2d\n", (_karel.x + 2) / 2, (_karel.y + 2) / 2, direction, _karel.beepers, _world.data[_karel.y][_karel.x]);
    ...
    for (int row = 0; row < _world.height; row++) {
      for (int col = 0; col < _world.width; col++) {
        if (_world.data[row][col] > 0) {
          ...
          printf("{\"x\": %d, \"y\": %d, \"count\": %d}", col / 2 + 1, row / 2 + 1, _world.data[row][col]);
        }
      }
    }
